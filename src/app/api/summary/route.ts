...
const headers = {
  'User-Agent': 'FilingViewer/1.0 (you@example.com)',
...
const getCIK = (ticker: string): string | null => {
  ...
  const tickers: Record<string, TickerEntry> = JSON.parse(raw); // 👈 define shape
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `Give me a short, clear, human-friendly summary of the following ${formType} SEC filing. Emphasize insights, risks, and financial info. Start with: "This is a summary of ${ticker}'s ${formType} filing." \n\n ${report}`
    });
